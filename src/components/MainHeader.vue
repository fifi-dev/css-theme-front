...
<template>
  <header class="p-0 flex justify-between items-center pb-4 max-sm:pb-8 max-sm:pt-4">
    ...
    <nav v-if="isMenuOpen"
         class=" w-fit flex items-center max-sm:fixed max-sm:w-full max-sm:bg-white max-sm:px-14 max-sm:py-48">
      <button @click="closeMenu" class="text-2xl hidden max-sm:block z-50 absolute text-primary top-8 right-10">x
      </button>
      ...
      <a @click="closeMenu" class="pr-8 font-semibold text-primary" href="https://github.com/fifi-dev/csstheme"
